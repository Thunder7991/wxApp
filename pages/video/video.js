...
Page({
  ...
  handleTimeUpdate(event) {
    ...
    if (videoTime) {
      videoTime.currentTime = event.detail.currentTime
    } else {
  ...
  handleEnded(event) {
    ...
    videoUpdateTime.splice(videoUpdateTime.find(item => item.vid === event.currentTarget.id), 1)
    this.setData({
      videoUpdateTime
    })
  },
  ...
  onShareAppMessage: function (from) {
    if (from === 'button') {
      return {
        ...
        iamgeUrl: '/static/images/IMG_20210217161940_CARP.jpg'
      }
    } else {
      return {
        title: '来自右上角',
